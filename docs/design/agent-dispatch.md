...
| protocol-designer | Claude agent, **fable** | wire shapes, MCP tool definitions, schema freezes (#9) | output requires Nick's D8 sign-off | high |
...
| external automation (Jules, codex cloud) | cloud, launched by Nick | parallel throughput on crisp, disjoint issues | no brief, no isolation guarantee — output enters as a PR and must pass the full review gate (`gate:reviewed` label) before merge; never shares a branch with an active dispatcher (P1: #60 clobbered three times) | medium |

## Decision table

| The issue is… | Dispatch to |
|---|---|
| a wire shape, schema version, or MCP tool definition | protocol-designer (fable) |
...
| auth, capability enforcement, or otherwise security-sensitive | server-engineer or principal — never codex full-auto; review gate adds a security-reviewer pass |
...
| ungated external-automation merges | #59 (reopened a real panic + broken lint config), #61 (verified post-merge only), #63 (MCP-only parity regression caught at gate) | externally-originated PRs get the five-gate review + `gate:reviewed` label before merge, enforced by `process-checks.yml` |
| concurrent automation clobbering a shared branch | #60 (overwritten mid-fix three times, one non-compiling push) | never push to a branch whose automation may still be running |
